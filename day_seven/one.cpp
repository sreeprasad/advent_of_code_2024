...
bool is_valid(ll lhs,vector<ll> rhs, ll value, int index){
  ...
  return is_valid(lhs,rhs,value+rhs[index],index+1) || is_valid(lhs,rhs,value*rhs[index],index+1);
}
...
void solve(){
  ...
  for(int i=0;i<len;i++){
    ...
    bool ok = is_valid(lhs,rhs,rhs[0]+rhs[1],2) || is_valid(lhs,rhs,rhs[0]*rhs[1],2);
    if(ok){
      result+=lhs;
    };
  }
